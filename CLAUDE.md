...
- Ширина `nodeW` динамическая: самый «широкий» слой умещается в ~4000px
  (`TARGET_TOTAL_W`), в диапазоне `NODE_MIN_W..NODE_MAX_W` (90..180)
...
### Взаимодействие
- **Клик по узлу** → кастомная подсветка **полного пути** (upstream + downstream
  через `adjUp`/`adjDown` + рекурсивный обход): узел увеличивается, предки —
  синяя обводка `#3b82f6`, потомки — зелёная `#22c55e`, рёбра пути жирные/синие,
  остальное гаснет (`opacity 0.1`). Управляется через `setOption` +
  `replaceMerge`, штатный `focusNodeAdjacency` на время отключается.
- **Двойной клик** → сброс к исходному состоянию (возврат `focusNodeAdjacency`).
